    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                ...
                const formattedData = products.map((p: any) => ({ ...p, id: p._id }));

                const currentPerfume = formattedData.find((p: any) => p.id === id);
                if (!currentPerfume) {
                    setLoading(false);
                    return;
                }
                ...
                const related = formattedData
                    .filter((p: any) => p.id !== currentPerfume.id)
                    .filter(
                        (p: any) =>
                            p.collectionName === currentPerfume.collectionName ||
